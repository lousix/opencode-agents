...
当项目为基于 Spring / Jalor 框架时，额外执行以下接口级规则，不依赖通用权限注解推断:

a. 枚举所有带 `@GetMapping/@PostMapping/@PutMapping/@DeleteMapping/@PatchMapping/@RequestMapping` 的 Controller 方法
b. **每一个接口都必须有 `@JalorOperation()`**
c. 对所有非 `GET` 接口，**必须有 `@ServiceAudit`**
d. 重点解析 `@ServiceAudit.message`，提取其中的参数占位符并与方法签名参数名逐一比对
e. 支持的临时占位形式: `{id}` / `${id}` / `#{id}`；若写为 `dto.id`，至少要求根参数 `dto` 存在
f. 对 `@RequestMapping` 未显式限定 HTTP method 的接口，按**可能包含非 GET**处理，要求补 `@ServiceAudit`

临时判定规则:
- 缺少 `@JalorOperation()` = **High（CWE-862）**
- 非 `GET` 缺少 `@ServiceAudit` = **Medium（CWE-778）**
- `@ServiceAudit.message` 为空 = **Medium（CWE-778）**
- `message` 引用的参数名与方法签名不一致 = **Medium（CWE-778）**
