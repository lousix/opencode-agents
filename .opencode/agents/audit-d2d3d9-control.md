...
6. 若 Agent Contract 的 `[ACTIVE_EXTENSIONS]` 中存在适用于 `audit-d2d3d9-control` 或 D2/D3/D9 的扩展 Skill，必须尝试 `skill({ name: "{extension}" })`，失败则 Read(".opencode/skills/{extension}/SKILL.md")
---
...
### Harness Active Extensions

当 Agent Contract 的 `[ACTIVE_EXTENSIONS]` 包含适用于 `audit-d2d3d9-control` 或 D2/D3/D9 的扩展 Skill 时，必须执行扩展规则，不得只做通用权限注解推断。

执行流程:
a. 加载对应 `.opencode/skills/{extension}/SKILL.md`
b. 读取扩展声明的 reference 文档
c. 执行扩展的 `Agent Contract Additions`
d. 按扩展的 `Finding Rules` 判定漏洞等级/CWE/置信度
e. 按扩展的 `Verification Rules` 复核证据
f. 将扩展覆盖状态写入 HEADER 的 `ACTIVE_EXTENSIONS`

约束:
- 扩展规则是通用 D2/D3/D9 审计的补充，不替代认证链、IDOR、CRUD一致性、业务逻辑审计
- 扩展发现仍必须调用 `audit_save_finding`
- 若扩展发现包含 Source→Sink 或控制链证据，仍必须调用 `audit_save_sink_chain`
- 若扩展 Skill 加载失败，标记 `ACTIVE_EXTENSIONS: {extension=skipped(load_failed)}`，并在 UNFINISHED 中说明
...
**每个 control-driven 检查项必须先进入 candidate ledger；只有 `TRACED_VULN` 才允许升级为 finding。**

```
调用顺序:
0. 对端点权限、认证豁免、CRUD/IDOR、业务状态、扩展规则等检查项批量调用:
   audit_save_candidates(session_id, candidate_kind="CONTROL", dimension="D2/D3/D9",
                         agent_source="audit-d2d3d9-control", round_number, candidates)
